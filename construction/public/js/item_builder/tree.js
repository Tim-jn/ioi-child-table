export default class ItemBuilderTree {
	constructor(opts) {
	...
	tree_methods() {
		...
		return {
			...
			toolbar: this.frm?.doc.docstatus === 0 ? [
				// {
				// 	label:__(frappe.utils.icon('edit', 'sm') + " " + __("Qty")),
				// 	click: function(node) {
				...
				{
					label:__(frappe.utils.icon('add', 'sm') + " " + __("Article composé")),
					click: function(node) {
						let view = frappe.views.trees["Quotation Builder"];
				...
				{
					label:__(frappe.utils.icon('move', 'sm') + " " +  __("Article composé")),
					click: function(node) {
						let view = frappe.views.trees["Quotation Builder"];
	...
	// 		frappe.call({
	// 			method: "construction.construction.quotation.treeview.edit_qty",
	// 			args: {
	// 				doctype: doctype,
	// 				docname: docname,
	// 				qty: data.qty,
