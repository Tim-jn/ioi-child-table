frappe.provide("construction.utils");
...
construction.utils.update_child_items = function (opts) {
	...
	const comments_and_titles = frm.doc[opts.child_docname].filter(d => d.row_type && !["item", ""].includes(d.row_type)).map(item => {
		return Object.assign(item, {docname: item.name})
	})

	this.data = frm.doc[opts.child_docname]
		.filter(d => !d.row_type || ["item", ""].includes(d.row_type))
		.map((d) => {
		return {
			docname: d.name,
			name: d.name,
