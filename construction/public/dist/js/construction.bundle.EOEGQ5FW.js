...
<svg class='tabulator-responsive-collapse-toggle-close' viewbox="0 0 24 24">
  ...
  var ResizeRows = class extends Module {
    ...
    _mouseDown(e, row, handle2) {
      ...
      self.table.element.classList.add("tabulator-block-select");
      function mouseMove(e2) {
        row.setHeight(self.startHeight + ((typeof e2.screenY === "undefined" ? e2.touches[0].screenY : e2.screenY) - self.startY));
      }
      function mouseUp(e2) {
  ...
  // ../construction/construction/public/js/item_builder/table.js
  var TABLE_COLUMNS = [
    "qty",
    "item_code",
    "item_name",
  ...
  var ItemBuilderTable = class {
    ...
    build_table() {
      ...
      this.table = new TabulatorFull(this.$table_wrapper.find(".tabulator-table")[0], tabulator_options);
    }
    get_columns() {
      const meta = frappe.get_meta("Quotation Item");
      let sort_keys = TABLE_COLUMNS;
      const filtered_fields = meta.fields.filter((f) => {
  ...
  var get_item_code_column = (mapped_column, col) => {
  ...
  var get_description_column = (mapped_column) => {
    mapped_column["editor"] = false;
    mapped_column["formatter"] = "html";
  };

  // ../construction/construction/public/js/item_builder/tree.js
  var ItemBuilderTree = class {
    constructor(opts) {
      Object.assign(this, opts);
      this.build_tree();
      this.bind_events();
    ...
    build_tree() {
      ...
      frappe.views.trees["Quotation Builder"] = new frappe.views.TreeView(options);
      this.tree_view = frappe.views.trees["Quotation Builder"];
    }
    tree_options() {
      return {
        parent: this.$tree_wrapper.get(0),
        body: this.$tree_wrapper.get(0),
        doctype: "Quotation",
        page: this.frm.page,
        expandable: true,
        title: __("Configure your quotation"),
        breadcrumb: "Selling",
        get_tree_nodes: "construction.construction.quotation.treeview.get_children",
        root_label: __("Items"),
    ...
    tree_methods() {
      ...
      return {
        onload: function(me) {
          ...
          me.parent = frm_obj.$tree_wrapper.get(0);
          me.body = frm_obj.$tree_wrapper.get(0);
          me.make_tree();
        },
        onrender(node) {
        ...
        toolbar: ((_a = this.frm) == null ? void 0 : _a.doc.docstatus) === 0 ? [
          {
            label: __(frappe.utils.icon("edit", "sm") + " " + __("Details")),
            click: function(node) {
              let view = frappe.views.trees["Quotation Builder"];
              view.events.edit_details(node, view);
            },
            btnClass: "hidden-xs"
          },
          {
            label: __(frappe.utils.icon("add", "sm") + " " + __("Item")),
            click: function(node) {
              let view = frappe.views.trees["Quotation Builder"];
          ...
          {
            label: __(frappe.utils.icon("add", "sm") + " " + __("Article compos\xE9")),
            click: function(node) {
              let view = frappe.views.trees["Quotation Builder"];
          ...
          {
            label: __(frappe.utils.icon("move", "sm") + " " + __("Article compos\xE9")),
            click: function(node) {
              let view = frappe.views.trees["Quotation Builder"];
    ...
    add_item(node, view) {
      frappe.prompt([
        ...
        frappe.call({
          method: "construction.construction.quotation.treeview.add_item",
          args: {
    ...
    delete_node(node, view) {
      frappe.confirm(__("Are you sure you want to delete this Item?"), () => {
        frappe.call({
          method: "construction.construction.quotation.treeview.delete_node",
          args: {
            parent: node.data.parent_id,
            fg_item: node.data.value,
    ...
    edit_details(node, view) {
      const data = node.data;
      const row = this.frm.fields_dict["items"].grid.grid_rows_by_docname[data.name];
      row.wrapper = node.$tree_link;
      row.show_form();
      node.$tree_link.data({
        grid_row: row,
        doc: this.frm.doc || ""
    ...
    load_tree(response, node) {
      ...
      frappe.views.trees["Quotation Builder"].tree.load_children(node);
      while (node) {
  ...
  // ../construction/construction/public/js/item_builder/index.js
...
//# sourceMappingURL=construction.bundle.EOEGQ5FW.js.map
